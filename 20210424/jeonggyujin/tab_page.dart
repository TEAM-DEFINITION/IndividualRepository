...
class _TabPageState extends State<TabPage> {
  ...
  _fetchpostcode() async {
    ...
    // 파잎에 아이디, 패스워드, 포스트코드, 해시 저장
    file.nextblockWrite_client(widget.userid, widget.userPwd, _postcode.text);
    // 파일 데이터를 암호화한 값을 encrypted에 저장
    final encrypted = await file.encrypting(widget.userid, widget.userPwd, _postcode.text);

    setState(() {
      isLoading = true;
    });
    // 서버의 /app/post 경로로 헤더 content-type, application/x-www-form-urlencoded, 바디는 id, pw, postcode를 보내는 post문 전달
    ...
    if (response.statusCode == 200) {
      ...
      // 서버로부터 아이디에 해당하는 body정보를 복호화해 _data에 저장
      final String _data = await file.decrypting(widget.userid, response.body);
      if (_data.split("|")[1] == "NULL") {// 인증서버|1|인증시간|3|
